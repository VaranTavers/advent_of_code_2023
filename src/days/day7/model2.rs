...
impl Hand {
    fn get_value_from_cards(cards: &Vec<Card>) -> usize {
        ...
        let jokers = cards_num[1];
        cards_num[1] = 0;

        let max = *cards_num.iter().max().unwrap_or(&0) + jokers;
        ...
        if max == 3 {
            if (jokers == 0 && cards_num.iter().any(|x| *x == 2))
                || (cards_num.iter().filter(|x| **x == 2).count() == 2 && jokers == 1)
            {
                return 4;
            }
    ...
    fn from_cards_and_bet(cards: Vec<Card>, bet: i64) -> Hand {
        Hand {
